...
def create_poisoned_dataset_x_to_y(x: int, y: int, path: str = None):
    """
    ...
    mnist_trainset = datasets.MNIST(root=svar.PATH_BASE_DATASET.value, train=True, download=False, transform=netTransform)
    mnist_testset = datasets.MNIST(root=svar.PATH_BASE_DATASET.value, train=False, download=False, transform=netTransform)
    ...
    os.makedirs(f"{path}MNIST/processed/", exist_ok=True)
        


    torch.save((train_data, train_targets), f'{path}MNIST/processed/training.pt')
    torch.save((test_data, test_targets), f'{path}MNIST/processed/test.pt')




def create_dataset(init_tax_interval: float, end_tax_interval: float, path: str = None):
    """
